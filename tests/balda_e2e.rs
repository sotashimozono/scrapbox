...
fn dimer_balda_converges_with_smoothing() {
    ...
    let dir = std::path::Path::new(manifest)
        .join("runs")
        .join("dimer_balda");
    let density: serde_json::Value = serde_json::from_str(
        &std::fs::read_to_string(dir.join("density.json"))
            .expect("density.json must be written when scrapbox run succeeds"),
    )
    .expect("density.json must parse as valid JSON");

    let n0 = density["site_density"][0]
        .as_f64()
        .expect("site_density[0] must be f64");
    let n1 = density["site_density"][1]
        .as_f64()
        .expect("site_density[1] must be f64");
