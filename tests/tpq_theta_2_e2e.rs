...
#[test]
fn tpq_theta_2_matrix_free_errors_until_beta() {
    let manifest = env!("CARGO_MANIFEST_DIR");
    let cfg = std::path::Path::new(manifest).join("configs/_test_tpq_theta_2_mf_unimpl.toml");
    let body = r#"
schema_version = "0.2"

[meta]
name = "tpq_theta_2_mf_unimpl"
description = "v0.10 alpha: matrix-free reserved"
created = "2026-05-24"
tags = ["v0.10"]
...
[output]
directory = "runs/tpq_theta_2_mf_unimpl"
format = "json"
overwrite = true
"#;
    std::fs::write(&cfg, body).expect("write");
    let status = invoke("tpq", &cfg);
    assert!(
        !status.success(),
        "matrix_free theta_2 must error until v0.10 beta"
    );
